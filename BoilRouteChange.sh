...
# 通过正则，只匹配末段以 0 结尾的地址，比如 192.168.51.10/24
# 这里用 (?<=inet\s) 确保只匹配 "inet x.x.x.x/xx" 的部分
# 再用 192\.168\.51\.\d+0(?=/) 来保证最后一位是 "0"
primary_ip=$(ip -4 addr show "$INTERFACE" \
    | grep -oP '(?<=inet\s)192\.168\.51\.\d+0(?=/)' \
    | head -n1)

if [ -z "$primary_ip" ]; then
    echo "未在 $INTERFACE 上找到 192.168.51.x0 格式的 IP！"
    echo "示例：192.168.51.10、192.168.51.20、192.168.51.30……"
    exit 1
fi

echo "检测到的主 IP：$primary_ip"

# base 去掉最后一个 0, 比如 192.168.51.10 => 192.168.51.1
base="${primary_ip%0}"
